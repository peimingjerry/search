...
int main() {
    BinarySearch<Person, long> myBinarySearch;
    Person *persons[SIZE];
    ...
    for (int i=0;i<SIZE;i++) {
        persons[i]->display();
        myBinarySearch.insert(*persons[i]);
    }
    cout<<endl;
    ...
    result = myBinarySearch.search(key);
    if (result != NULL) {
        cout<<"Found!"<<endl;
        result->display();
    } else
